...
export function Navbar() {
  ...
  return (
    <motion.header
      ...
      className="fixed inset-x-0 top-0 z-50 overflow-x-hidden"
    >
      <div
        className={cn(
          "mx-auto flex items-center justify-between gap-3 px-3 py-2.5 transition-all duration-500 sm:px-5 sm:py-3 md:px-8 md:py-3.5",
          scrolled
            ? "my-2 rounded-2xl border border-line bg-ink-900/70 backdrop-blur-xl shadow-soft"
            : "border border-transparent bg-transparent"
        )}
        style={{ maxWidth: scrolled ? 1180 : "100vw" }}
      >
        ...
        <button
          className="lg:hidden flex items-center justify-center rounded-lg border border-line p-2.5 text-white/80 shrink-0 min-h-[44px] min-w-[44px]"
          onClick={() => setOpen((o) => !o)}
